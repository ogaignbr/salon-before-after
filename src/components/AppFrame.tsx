...
export function AppHeader({ title, onBack, backLabel = '戻る', rightSlot }: AppHeaderProps) {
  return (
    <div className="mb-4 flex items-center rounded-[16px] border border-[#B9A7FF]/30 bg-[rgba(255,255,255,0.92)] px-3 py-2.5 shadow-[0_12px_32px_rgba(85,70,180,0.12)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/45 dark:shadow-[0_14px_38px_-28px_rgba(18,32,77,0.9)]">
      {onBack ? (
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-semibold text-[#6B4CFF] transition-colors hover:text-[#8B5CFF] dark:text-indigo-300 dark:hover:text-indigo-200"
        >
      ...
      <h1 className="flex-1 text-center text-sm font-bold tracking-[0.03em] text-[#161B5C] dark:text-slate-100">{title}</h1>
